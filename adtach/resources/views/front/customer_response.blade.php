        ...
        <table class="w-full table-auto border-collapse border border-gray-200">
            <thead>
                <tr class="bg-gray-100 text-gray-700">
                    ...
                    <th class="px-4 py-2 border border-gray-300">AGNET</th>
                    <th class="px-4 py-2 border border-gray-300">DATE</th>
                </tr>
            </thead>
            <tbody id="tableBody">
                @foreach ($customerResponse as $index => $customer)
                    <tr class="odd:bg-gray-50 even:bg-white">
                        ...
                        <td class="px-4 py-2 border border-gray-300 customer hidden sm:table-cell">{{ \Carbon\Carbon::parse($customer->date)->format('d M, Y') }}</td>
                    </tr>
                @endforeach
                @if ($customerResponse->isEmpty())
                    <tr>
                        <td colspan="10" class="text-center">No Customer Response Record Found</td>
                    </tr>
                @endif
            </tbody>
        </table>
